...
def test_sunrise_sunset_equator():
    ...
    astroplan_next_sunrise = obs.sun_rise(time, which='next').datetime
    astroplan_next_sunset = obs.sun_set(time, which='next').datetime

    astroplan_prev_sunrise = obs.sun_rise(time, which='previous').datetime
    astroplan_prev_sunset = obs.sun_set(time, which='previous').datetime
...
def test_vega_rise_set_equator():
    ...
    obs = Observer(location=location, pressure=pressure)
    astroplan_next_rise = obs.target_rise(time, vega, which='next').datetime
    astroplan_next_set = obs.target_set(time, vega, which='next').datetime

    astroplan_prev_rise = obs.target_rise(time, vega, which='previous').datetime
    astroplan_prev_set = obs.target_set(time, vega, which='previous').datetime

    astroplan_nearest_rise = obs.target_rise(time, vega, which='nearest').datetime
    astroplan_nearest_set = obs.target_set(time, vega, which='nearest').datetime
...
def test_sunrise_sunset_equator_civil_twilight():
    ...
    horizon = -6*u.degree
    astroplan_next_sunrise = obs.sun_rise(time, which='next',
                                         horizon=horizon).datetime
    astroplan_next_sunset = obs.sun_set(time, which='next',
                                       horizon=horizon).datetime

    astroplan_prev_sunrise = obs.sun_rise(time, which='previous',
                                         horizon=horizon).datetime
    astroplan_prev_sunset = obs.sun_set(time, which='previous',
                                       horizon=horizon).datetime
...
def test_solar_transit():
    ...
    astroplan_next_transit = obs.target_meridian_transit(time, get_sun(time),
                                                       which='next').datetime
    astroplan_next_antitransit = obs.target_meridian_antitransit(time,
                                                               get_sun(time),
                                                               which='next').datetime
    astroplan_prev_transit = obs.target_meridian_transit(time, get_sun(time),
                                                       which='previous').datetime
    astroplan_prev_antitransit = obs.target_meridian_antitransit(time,
                                                               get_sun(time),
                                                               which='previous').datetime

    astroplan_nearest_transit = obs.target_meridian_transit(time, get_sun(time),
                                                          which='nearest').datetime
    astroplan_nearest_antitransit = obs.target_meridian_antitransit(time, get_sun(time),
                                                                  which='nearest').datetime
...
def test_TargetAlwaysUpWarning(recwarn):
    ...
    obs = Observer(location=location)
    no_time = obs.target_rise(time, polaris, which='next')
...
class TestExceptions(unittest.TestCase):
    def test_rise_set_transit_which(self):
        ...
        with self.assertRaises(ValueError):
            obs.target_rise(time, vega_coords, which='oops').datetime

        with self.assertRaises(ValueError):
            obs.target_set(time, vega_coords, which='oops').datetime

        with self.assertRaises(ValueError):
            obs.target_meridian_transit(time, vega_coords, which='oops').datetime

        with self.assertRaises(ValueError):
            obs.target_meridian_antitransit(time, vega_coords, which='oops').datetime
